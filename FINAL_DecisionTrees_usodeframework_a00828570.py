...
test_pred = clf_tree_iris.predict(X_test_iris)
...
print(' SepalLengthCm | SepalWidthCm |  PetalLengthCm | PetalWidthCm | ACTUAL | PREDICTED')
for i in range(5):
...
df1.Classification.replace([1,2,3], ['Cultivar 1', 'Cultivar 2', 'Cultivar 3'], inplace=True)
df1.Classification
...
df_x = df1.drop(["Classification"], axis=1).values
...
import numpy as np
np.set_printoptions(suppress=True) # Para que los valores no salgan como notación científica

#X_test

df1.columns

# Se imprimen primero los títulos de cada variables
# Después los valores de cada variable
# Y para cada conjunto de variables de entrada se imprime el valor esperado junto con el valor predicho
# 'Real' hace referencia a valor esperado
# 'Pred' hace referencia al valor predicho 

print('Alcohol', 'Malic acid', 'Ash', 'Alcalinity of ash','Magnesium', 'Total phenols', 'Flavanoids', 'Nonflavanoid phenols','Proanthocyanins', 'Color intensity', 'Hue',
       'OD280/OD315 of diluted wines', 'Proline')
for i in range(5):
  print(i, X_test[i], 'Real: ',y_test_list[i],' | Pred: ', test_pred_decision_tree_list[i])
